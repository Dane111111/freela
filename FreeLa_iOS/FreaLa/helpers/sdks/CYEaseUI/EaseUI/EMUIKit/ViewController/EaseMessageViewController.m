...
- (void)beKickedOutFromChatroom:(EMChatroom *)chatroom reason:(EMChatroomBeKickedReason)reason
{
    if ([_conversation.chatter isEqualToString:chatroom.chatroomId])
    {
        _isKicked = YES;
        CGRect frame = self.chatToolbar.frame;
        [self showHint:[NSString stringWithFormat:@"从聊天室被移除\'%@\'", chatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            [weakSelf showHudInView:self.view hint:@"录制时间太短"];
                     ...
                     progress:(id<IEMChatProgressDelegate>)progress
{
    ...
    NSString* xjUserName = [[NSUserDefaults standardUserDefaults] objectForKey:XJ_FOR_HUANXIN_USERNAME];
    NSString* xjAvatar = [[NSUserDefaults standardUserDefaults] objectForKey:XJ_FOR_HUANXIN_AVATAR];
    [xjDic setObject:xjUserName?xjUserName:@"" forKey:XJ_HUANXIN_KEY_USERNAME];
    [xjDic setObject:xjAvatar?xjAvatar:@"" forKey:XJ_HUANXIN_KEY_AVATAR];
