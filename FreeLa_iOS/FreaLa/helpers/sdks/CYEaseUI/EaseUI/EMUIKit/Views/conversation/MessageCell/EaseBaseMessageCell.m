                        ...
                        model:(id<IMessageModel>)model
{
    ...
    if (self) {
        ...
        self.contentView.backgroundColor = [UIColor colorWithRed:230 / 255.0 green:230 /255.0 blue:230 / 255.0 alpha:1];
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    CYUserModel *userModel = [userProfileTool modelWithUserId:model.nickname];
    if (userModel) {
    ...
    //    如果是sender
    if (model.isSender) {
        if (model.avatarURLPath) {  // 如果有网络图片
            
            NSString *imageURL = [XJFinalTool xjReturnImageURLWithStr:model.avatarURLPath isSite:NO];
            if (![FLTool returnBoolWithIsHasHTTP:model.avatarURLPath  includeStr:@"http://"]) {
                imageURL = [FLBaseUrl stringByAppendingPathComponent:model.avatarURLPath];
            }
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:imageURL] placeholderImage:[UIImage imageNamed:@"defaultavater"]];
        } else { // 如果没有
        ...
        if (model.avatarURLPath) {  // 如果有网络图片
            NSString *imageURL = model.avatarURLPath;
            if (![FLTool returnBoolWithIsHasHTTP:model.avatarURLPath  includeStr:@"http://"]) {
                imageURL = [FLBaseUrl stringByAppendingPathComponent:model.avatarURLPath];
            }
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:imageURL] placeholderImage:[UIImage imageNamed:@"defaultavater"]];
