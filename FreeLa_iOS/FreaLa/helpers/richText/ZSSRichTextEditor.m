...
- (void)viewDidLoad {
    ...
    // Source View
    CGRect frame = CGRectMake(0, StatusBar_NaviHeight, self.view.frame.size.width, self.view.frame.size.height +StatusBar_NaviHeight );
...
- (void)showInsertImageDialogWithLink:(NSString *)url alt:(NSString *)alt {
    ...
    // Insert Button Title
//    NSString *insertButtonTitle = !self.selectedImageURL ? NSLocalizedString(@"Insert", nil) : NSLocalizedString(@"Insert", nil);
...
//add
- (void)insertImage:(NSString *)url alt:(NSString *)alt xjWandH:(NSString*)xjStr {
    NSString *trigger = [NSString stringWithFormat:@"zss_editor.insertImage(\"%@\", \"%@\",\"%@\");", url, alt,xjStr];
    [self.editorView stringByEvaluatingJavaScriptFromString:trigger];
}
...
- (void)textViewDidChange:(UITextView *)textView {
    ...
    if ( overflow > 0 ) {
        ...
        FL_Log(@"this is a a a a a a a =%f",offset.y);
...
//    NSString* ff = [NSString stringWithFormat:@"%ld",position];
//    [self.editorView.scrollView setContentOffset:CGPointMake(0, [ff floatValue]) animated:YES];
...
//            self.editorView.scrollView.scrollIndicatorInsets = UIEdgeInsetsZero;
            ...
            //add
            self.editorView.scrollView.contentSize = CGSizeMake(FLUISCREENBOUNDS.width, FLUISCREENBOUNDS.height + sourceFrame.size.height);
//            [self.editorView.scrollView setContentOffset:CGPointMake(0, sourceFrame.size.height) animated:YES];
            
            // Provide editor with keyboard height and editor view height
            [self setFooterHeight:(keyboardHeight - 8)];
            [self setContentHeight: self.editorViewFrame.size.height - StatusBar_NaviHeight];
