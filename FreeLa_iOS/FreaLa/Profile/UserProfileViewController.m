...
- (UIImageView*)headImageView
{
    ...
    CYUserProfileTool *userProfile = [CYUserProfileTool share];
    CYUserModel *userModel = [userProfile modelWithUserId:_username];
    if (userModel) {
        NSString *imageURL = [XJFinalTool xjReturnImageURLWithStr:userModel.avatar isSite:NO];//[FLBaseUrl stringByAppendingPathComponent:userModel.avatar];
        [_headImageView sd_setImageWithURL:[NSURL URLWithString:imageURL] placeholderImage:[UIImage imageNamed:@"defaultavater"]];
    }
    return _headImageView;
}

- (void)setXjAvatarStr:(NSString *)xjAvatarStr {
    if (![FLTool returnBoolWithIsHasHTTP:xjAvatarStr includeStr:@"http://"]&&xjAvatarStr) {
        xjAvatarStr = [FLBaseUrl stringByAppendingString:xjAvatarStr];
    }
    [self.headImageView sd_setImageWithURL:[NSURL URLWithString:xjAvatarStr]];
}
