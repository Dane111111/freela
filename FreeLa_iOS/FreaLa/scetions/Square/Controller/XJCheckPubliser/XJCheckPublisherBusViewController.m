//
//  XJCheckPublisherBusViewController.m
//  FreeLa
//
//  Created by Leon on 16/6/13.
...
@interface XJCheckPublisherBusViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
{
...
@implementation XJCheckPublisherBusViewController

- (instancetype)initWithUserId:(NSString*)xjstr {
    self = [super init];
    if (self) {
        self.xjUserIdStr = xjstr;
        [self seeAllUserInfo];
    }
    return self;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetY > NAVBAR_CHANGE_POINT) {
        ...
        self.title = self.busAccountModel.busSimpleName;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.xjTableViewAllFree)  {
        ...
        FLFuckHtmlViewController*  detailVC  = [[FLFuckHtmlViewController alloc] init];
        //此处模型为详情model
        FLMyIssueInMineModel* xjModel = self.xjIssueAllFreeMuArr.count >= indexPath.row ? self.xjIssueAllFreeMuArr[indexPath.row]:nil;
        detailVC.flFuckTopicId = xjModel.flMineIssueTopicIdStr;
        ...
        if (self.xjIssueCouponMuArr.count >= indexPath.row) {
            ...
            receiveVC.flFuckTopicId = xjReceiveModel.flMineIssueTopicIdStr;
...
- (void)getModelsInMineVCWithCurrentPage:(NSNumber*)currentPage {
    ...
    [FLNetTool searchTopicListByIDWithParm:parm success:^(NSDictionary *data) {
        FL_Log(@"data in test mysversion2 issue=%@",data);
...
- (void)creatScrollView {
    ...
    scrollView.contentSize = CGSizeMake(FLUISCREENBOUNDS.width , 0);
...
//    [self.myNameButton setTitle:@"nickName" forState:UIControlStateNormal];
    ...
    //添加好友btn
    self.xjAddBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40,40)];
    UIBarButtonItem* xjBarItem = [[UIBarButtonItem alloc] initWithCustomView:self.xjAddBtn];
    self.navigationItem.rightBarButtonItem = xjBarItem;
...
- (void)seeAllUserInfo {
    NSDictionary* parmBus =@{@"userid": self.xjUserIdStr ? self.xjUserIdStr : @"",
...
- (void)getFriendListInBusMinePage{
    if (![[EaseMob sharedInstance].chatManager  isLoggedIn]) {
        //自动登录环信
        NSString *userName = [NSString stringWithFormat:@"%@_%@",XJ_USERTYPE_WITHTYPE,self.xjUserIdStr ? self.xjUserIdStr :@""];
...
-(void) xjAddPublisher {
    ...
    NSDictionary* parm = @{@"owerName":[NSString stringWithFormat:@"%@_%@",XJ_USERTYPE_WITHTYPE,XJ_USERID_WITHTYPE],
                           @"friendName":[NSString stringWithFormat:@"%@_%@",FLFLXJUserTypeCompStrKey,self.xjUserIdStr]};
    [FLNetTool xjAddFriendsByParm:parm success:^(NSDictionary *data) {
        FL_Log(@"this is the result t2o add friend=%@",data);
...
- (void)xjCheckIsFriend {
    NSDictionary* parm = @{@"imOwerName":[NSString stringWithFormat:@"%@_%@",XJ_USERTYPE_WITHTYPE,XJ_USERID_WITHTYPE],
                           @"imFriendName":[NSString stringWithFormat:@"%@_%@",FLFLXJUserTypeCompStrKey,self.xjUserIdStr]};
    [FLNetTool xjCheckIsFriendsByParm:parm success:^(NSDictionary *data) {
        FL_Log(@"this is th3e result to check friend=%@",data);
        if ([data[FL_NET_KEY_NEW] boolValue]) {
...
- (void)xjRemoveFriend {
    EMError *error;
    NSString* xjStr = [NSString stringWithFormat:@"%@_%@",FLFLXJUserTypeCompStrKey,self.xjUserIdStr];
