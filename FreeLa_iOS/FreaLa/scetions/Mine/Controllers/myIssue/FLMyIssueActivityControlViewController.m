//
//  FLMyIssueActivityControlViewController.m
//  FreeLa
//
...
@interface FLMyIssueActivityControlViewController ()<UIScrollViewDelegate>
{
    FLMyIssueControlDetailViewController *chatVC;
    FLMyIssueReceiveViewController *contactVC;
    FLMyIssueJudgePLViewController *attentionVC;
    FLMyIssueJudgePJViewController *recommendVC;
...
@implementation FLMyIssueActivityControlViewController
...
//    [self setupContentView];// UIScrollView
    ...
    self.title = self.flmyIssueInMineModel.flMineTopicThemStr;
...
- (void)setFlIssueInfoModelDic:(NSDictionary *)flIssueInfoModelDic {
    _flIssueInfoModelDic = flIssueInfoModelDic;
//    chatVC.xj_tempId = @"";
}
...
- (void)setupChildVC{
     ...
     chatVC = [[FLMyIssueControlDetailViewController alloc] init];
    chatVC.xjTioicId = _xjTopicId;
    if (_flmyIssueInMineModel.xjTempId) {
        chatVC.xj_tempId =_flmyIssueInMineModel.xjTempId;
    }
    [self addChildViewController:chatVC];
    
    contactVC = [[FLMyIssueReceiveViewController alloc] init];
    contactVC.xjTopicIdStr = _xjTopicId;
    [self addChildViewController:contactVC];
    
     attentionVC = [[FLMyIssueJudgePLViewController alloc] init];
    attentionVC.xjTopicIdStr = _xjTopicId;
    [self addChildViewController:attentionVC];
    
    recommendVC = [[FLMyIssueJudgePJViewController alloc] init];
    recommendVC.xjTopicIdStr = _xjTopicId;
...
-(void)setupTitleView{
    ...
    NSArray *titleStrArr = @[@"管理",@"领取",@"评论",@"评价"];
...
//    titleView.backgroundColor =  // [UIColor colorWithHexString:@""];
    titleView.backgroundColor = [UIColor colorWithRed:249 green:249 blue:249 alpha:1];
    self.titleView = titleView;
    [self.view addSubview:titleView];
    
    CGFloat btnX = self.view.cy_width / 4 / 2;
    CGFloat btnY = 0;
    ...
    for (int i = 0; i < titleStrArr.count; i ++) {
        ...
        [btn sizeToFit];
        btn.cy_centerX = btnX + i * (self.view.cy_width / 4);
        btn.cy_y = btnY;
