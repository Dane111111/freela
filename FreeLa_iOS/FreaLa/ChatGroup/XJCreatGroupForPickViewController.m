//
//  XJCreatGroupForPickViewController.m
//  FreeLa
//
...
@interface XJCreatGroupForPickViewController ()<UITextFieldDelegate, UITextViewDelegate, EMChooseViewDelegate>
...
@implementation XJCreatGroupForPickViewController
...
- (void)viewDidLoad {
    ...
    UIView* xjCover = [[UIView alloc] initWithFrame:CGRectMake(0, 0, FLUISCREENBOUNDS.width, FLUISCREENBOUNDS.height)];
    [self.view addSubview:xjCover];
    xjCover.backgroundColor = [UIColor colorWithRed:0.88 green:0.88 blue:0.88 alpha:1.0];
...
- (BOOL)viewController:(EMChooseViewController *)viewController didFinishSelectedSources:(NSArray *)selectedSources
{
    ...
    __weak XJCreatGroupForPickViewController *weakSelf = self;
...
- (void)addContacts:(id)sender
{
    ...
    for (NSString*xj in self.xjUserIdArr) {
        NSString* ss  = @"person_";
        NSString* userId = [NSString stringWithFormat:@"%@%@",ss,xj];//[ss stringByAppendingString:xj];
        
        EMBuddy* body = [EMBuddy buddyWithUsername:userId];
        [xjArr addObject:body];
    }
    [self viewController:nil didFinishSelectedSources:xjArr.mutableCopy];
